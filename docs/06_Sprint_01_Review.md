...
- 首頁左上優先顯示淨資產，並以「淨資產＝總資產－總負債」呈現計算關係。
- 資產與負債在同一頁分組呈現，各組顯示列入首頁的小計與獨立空狀態。
- 顯示啟用中的資產／負債、類型、資料狀態、金額及更新時間。
- 支援新增與編輯資產／負債。
- 支援設定資料狀態及是否列入個人資產／負債。
- 尚未關聯歷史紀錄的項目可在確認後永久刪除。
- 金額欄位預設空白、無上下箭頭，且不允許新增或儲存零元項目。
- 單筆金額上限為 `TWD 999,999,999,999`，超過時以短暫提示說明，不顯示內部 IPC 錯誤。
- 所有首頁金額明確使用銀行常見的 `TWD`，更新時間包含西元年份。
- 非必要介面文字使用中文。
- 「列入我的資產／負債」依項目方向動態顯示，並收在更多設定中。
- 名稱為選填；留空時自動使用類型名稱，並允許不同項目使用相同名稱。
