...
- 依據文件：`00_Architecture_Rules.md` v1.1、`00_Glossary.md` v1.1、`04_Decision_Log.md`
...
- 加入後檢查工作區：**若 `git status` 仍然乾淨，就到此為止，不要製造任何正規化 commit。**
- 若加入 `.gitattributes` 後真的出現差異，先回報實際情況再決定，不要自行執行全庫改寫。
...
完成 T-00 至 T-03 與 T-05 後，合併 `codex/sprint-02` 至 main，再由 main 開出 `codex/sprint-02-5`。
...
- renderer 內不存在金額加總、金額比較或收支類型判斷。
- 每日與月度統計對同一批資料的判斷結果一致，並有測試證明。
- 依 Architecture Rules 1.4 的界線，純計數（筆數、字數）留在 renderer 是允許的，不需上移。
...
- 在 v4 區塊加註解，說明它是規範建立前的歷史例外。
- 兩項都在 `04_Decision_Log.md` 記錄（T-03 已含）。
